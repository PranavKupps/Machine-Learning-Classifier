...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    int lc = size_impl(node->left);
    int rc = size_impl(node->right);
  ...
  static int height_impl(const Node *node) {
    ...
    int lc_height = height_impl(node->left);
    int rc_height = height_impl(node->right);

    return 1 + std::max(lc_height, rc_height);
  }
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    destroy_nodes_impl(node->left); //delete left side of tree
    destroy_nodes_impl(node->right); //delete right side of tree

    delete node; //actuall deletes the node
    node = nullptr;
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if(empty_impl(node)) {
      ...
      return true;
               ...
               (less(node->datum, node->left->datum))) {
        return false;
    } else if((node->right != nullptr) && 
    ...
    bool lc = check_sorting_invariant_impl(node->left, less);
    bool rc = check_sorting_invariant_impl(node->right, less);
    return (lc && rc);
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (node == nullptr) {
      return nullptr;
    } else if (less(max_element_impl(node)->datum, val)) {
      return nullptr;
    } else if (less(node->datum, val)) {
      return min_greater_than_impl((node->right), val, less);
    } else if (less(val, node->datum)) {
      Node *node_l = min_greater_than_impl(node->left, val, less);
      if (node_l != nullptr) {
        return node_l;
      } else {
        return node;
