...
public:
  ...
  Iterator find(const Key_type& k) const {
    pair<Key_type, Value_type> node1;
    node1.first = k;
    node1.second = Value_type();
    return tree.find(node1);
  }
  ...
  Value_type& operator[](const Key_type& k) {
    Iterator node1 = find(k);
    pair<Key_type, Value_type> node2;
    node2.first = k;
    node2.second = Value_type();
    
    if(tree.find(node2) == tree.end()) {
      node1 = tree.insert(node2);
      return node1->second;
    } else {
      return node1->second;
    }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    auto node1 = tree.find(val);
    pair<Iterator, bool> return_value;

    if(node1 != tree.end()) {
      Iterator node = tree.find(val);
      return_value.first = node;
      return_value.second = false;
      return return_value;
    } else {
      Iterator node = tree.insert(val);
      return_value.first = node;
